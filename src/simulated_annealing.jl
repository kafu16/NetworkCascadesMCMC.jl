...
function parallel_multiple_sim_anneal_old(directory::String, g::AbstractGraph, P_inits::Vector{Any}, C::AbstractFloat, annealing_schedule::Function, annealing_schedule_name::String, steps_per_temp::Integer, k_max::Integer, N_runs::Integer)
    Data = @distributed vcat for i in 1:N_runs
        sim_anneal_old(g, P_inits[i], C, annealing_schedule, steps_per_temp, k_max)
        #P, en = sim_anneal(g, P_inits[i], C, annealing_schedule, steps_per_temp, k_max)
        #push!(energies, en)
        #push!(P_finals, P)
    end
...
function temp_ex2_a(k::Integer, steps_per_temp::Integer)
...
function temp_ex3_a(k::Integer, steps_per_temp::Integer)
    2. * 0.99 ^ (floor(k/steps_per_temp)) # floor(x) returns the nearest integral value of the same type as x that is less than or equal to x
end

function temp_ex3_b(k::Integer, steps_per_temp::Integer)
...
function temp_ex5_a(k::Integer, steps_per_temp::Integer)
    10. * 0.99 ^ (floor(k/steps_per_temp)) # floor(x) returns the nearest integral value of the same type as x that is less than or equal to x
end
